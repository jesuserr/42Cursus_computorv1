...
def pre_parser(arguments):
    ...
    if (len(identity_sides) != 2 or not identity_sides[0]
            or not identity_sides[1]):
...
def free_form_parser(term, coefficients, side):
    if re.search(r'^([+-]?\d*\.?\d*)$', term):       # Single coeff. ("4")
        ...
        if ((side == "right_term" and match[0] == "-")
                or (side == "left_term" and match[0] != "-")):
            coefficients[1] += 1
        else:
            coefficients[1] -= 1
        ...
        coefficient = 1
        if ((side == "right_term" and match[0][0] != "-")
                or (side == "left_term" and match[0][0] == "-")):
            coefficient = -1
