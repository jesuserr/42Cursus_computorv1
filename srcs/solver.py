...
def print_irreducible_complex(
        numerator1, numerator2, denominator, discriminant, b):
...
def print_results(a, b, c, irreducible):
    ...
    print(f"{BLUE}", end="")
    if discriminant > 0:
        print(f"Discriminant is strictly positive, the two solutions "
              f"are:{DEF}")
    elif discriminant == 0:
        print(f"Discriminant is zero, the two solutions are identical:{DEF}")
    else:
        print("Discriminant is strictly negative, ", end="")
...
def solver(coefficients, steps, irreducible):
    ...
    if (len(coefficients) > 3
            and not all(coef == 0 for coef in coefficients[3:])):
    ...
    if all(coef == 0 for coef in coefficients):
        raise ValueError(
            f"{BLUE}Polynomial degree: {DEF}" + "Not Applicable"
...
def print_intermediate_steps(a, b, c, type):
    ...
    if type == "linear":
        print("Linear formula".ljust(30) + "->\t" + "x = -b / a")
        print("Linear equation".ljust(30) + "->\t" + f"{b}x + {c} = 0")
        print("Coefficients".ljust(30) + "->\t" + "a =", b, ", b =", c)
        print("Solution".ljust(30) + "->\t" + f"x = -({c}) / {b} = ", end="")
        print(f"{strip(c / -b)}")
        return
    discriminant = (b ** 2) - (4 * a * c)
    numerator1 = -b - (discriminant) ** 0.5
    numerator2 = -b + (discriminant) ** 0.5
    print("Quadratic formula".ljust(30) + "->\t"
          + "x = (-b ± √(b² - 4ac)) / 2a")
    print("Quadratic equation".ljust(30) + "->\t"
          + f"{a}x² + {b}x + {c} = 0")
    print("Coefficients".ljust(30) + "->\t"
          + "a =", a, ", b =", b, ", c =", c)
    print("Discriminant (Δ = b² - 4ac)".ljust(30) + "->\t", end="")
    print(f"Δ = {b}² - 4 * {a} * {c} =",
          f"{b ** 2} - ({4 * a * c}) =", end="")
    print(f" {discriminant}")
    print("First solution".ljust(30) + "->\t" + f"x1 = (-({b}) - ", end="")
    print(f"√{discriminant}) / {2 * a} =", f"{strip(numerator1 / (2 * a))}")
    print("Second solution".ljust(30) + "->\t" + f"x2 = (-({b}) + ", end="")
