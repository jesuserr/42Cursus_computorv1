...
# To use fractions module instead of this function see below
# add "from fractions import Fraction"
# use "return (f"{Fraction(number).limit_denominator()}")"
...
def parse_arguments():
    # Trick to avoid argparse error for negative numbers on equation
    # with no spaces
    if len(sys.argv) > 1:
